...
let img_logo = document.querySelector('.loader img');
if (loader) {
    setTimeout(function () {
...
chat_header.classList.add('hide');
...
let user_inp = document.querySelector('.user');
var userID = localStorage.getItem('userID');
...
getJsonValue('accounts').then(chats => {
    let accounts = chats;
    Object.entries(accounts).forEach(([key, value]) => {
        if (value.ID == userID) {
            user_inp.value = value.Name;
        }
    });
});

// Адаптация для мобильных: скрываем клавиатуру при клике вне инпута
document.addEventListener('click', function (event) {
    if (event.target.id !== 'message-input' && event.target.id !== 'send-button') {
        document.getElementById('message-input')?.blur();
    }
});

// Автоматический скролл при фокусе на инпуте на мобильных
            ...
            <div class="modal-content">
                <form id="createChatForm">
                    ...
                    <div class="rules-section">
                        <label>Правила чата:</label>
                        <div class="users-selection">
                        <label class="rule-option">
                            <input checked type="checkbox" id="ruleAllCanSend" name="ruleAllCanSend">
                            <span>Все могут отправлять сообщения</span>
                        </label>
                    </div>
                    </div>
...
function displayChats(chats) {
    ...
    const headerHTML = `
    <div class='header'>
    <img style='width:50px;height:50px;' src='static/img/favicon.png'>
    <h2>ChatApp</h2>
    ...
    chats.forEach(async chat => {
        ...
        chatElement.innerHTML = `
            <div class="chat-info">
                ...
                <p class="id">${(await apiRequest(`/api/get_messages/${chat.id}`)).messages.pop()?.sender} : ${((await apiRequest(`/api/get_messages/${chat.id}`)).messages?.at(-1)?.text || '').substring(0, 40) + (((await apiRequest(`/api/get_messages/${chat.id}`)).messages?.at(-1)?.text || '').length > 40 ? '...' : '')}</p>
            </div>
        `;
...
async function openChat(id) {
    ...
    chat_header.classList.remove('hide');
...
function closechat() {
    setTimeout(() => {
        ...
        if (!phoneMode) {
            ...
            msg_list.innerHTML = ''
            chatId.textContent = ''
            chatName.textContent = 'Чат не выбран!'
            chat_header.classList.add('hide');
            form.classList.add('hide');
        }
